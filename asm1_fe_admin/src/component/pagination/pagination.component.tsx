...
export const Pagination: React.FC<Props> = observer(() => {
  const total = usersStore.lengthOfUsers;

  const numberOfPage = useMemo(() => {
    return Math.ceil(total / usersStore.sizeOfPages);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [total, usersStore.sizeOfPages]);

  const handleChangeSize = (e: React.ChangeEvent<HTMLSelectElement>) => {
    usersStore.setPageSize(parseInt(e.target.value));
    usersStore.setCurrentPage(1); // Reset to the first page when the size changes
  };

  const handlePageChange = (newPage: number) => {
    if (newPage > 0 && newPage <= numberOfPage) {
      usersStore.setCurrentPage(newPage);
    }
  };
  ...
  return (
    <nav className="flex flex-wrap justify-between items-center mb-4">
      <form className="max-w-sm">
        ...
        <select
          id="unitPerPage"
          defaultValue={usersStore.sizeOfPages}
      ...
      <ul className="inline-flex -space-x-px text-sm">
        <li>
          <div
            onClick={() => handlePageChange(usersStore.currentPage - 1)}
          ...
          <li key={index}>
            <div
              ...
              className={`cursor-pointer flex items-center justify-center px-3 h-8 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white ${
                usersStore.currentPage === index + 1
